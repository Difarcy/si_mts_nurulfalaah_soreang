...
@section('title', 'Tambah Video Kegiatan')

@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Tambah Video Kegiatan</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Masukkan link video Youtube kegiatan sekolah.</p>
            </div>
            <div>
        ...
        <div class="bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700">
            <form action="{{ route('cpanel.activity-videos.store') }}" method="POST" id="activity-video-form"
                data-notify="loading" data-unsaved-changes="true" class="p-6">
                @csrf
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="url" name="youtube_url" id="youtube_url" value="{{ old('youtube_url') }}" required
                                maxlength="255" placeholder="Masukan Link"
                                class="w-full border-2 border-gray-200 dark:border-slate-600 px-3 py-1.5 text-base bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:border-green-600 focus:outline-none rounded">
                        </div>
