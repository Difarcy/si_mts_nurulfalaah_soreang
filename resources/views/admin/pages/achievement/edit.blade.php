...
@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Edit Prestasi Siswa</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Perbarui informasi prestasi siswa dan kelola
                    gambar dokumentasi.</p>
            </div>
        ...
        <form action="{{ route('cpanel.prestasi-siswa.update', $prestasiSiswa) }}" method="POST"
            enctype="multipart/form-data" id="student-achievement-form" data-notify="loading" data-unsaved-changes="true"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2 space-y-6">
                    <div>
                        <label for="gambar" class="block text-base font-semibold text-slate-700 dark:text-white mb-1">Upload
                        ...
                        <div id="drop-zone-gambar-prestasi"
                            ...
                            <div id="upload-placeholder-gambar-prestasi"
                                class="text-center space-y-3 w-full h-full flex flex-col items-center justify-center p-8 {{ $prestasiSiswa->gambar ? 'hidden' : '' }}">
                            ...
                            <div id="preview-container-gambar-prestasi"
                                class="{{ $prestasiSiswa->gambar ? '' : 'hidden' }} w-full h-full p-2">
                                <div class="relative group h-full">
                                    <div class="overflow-hidden border border-gray-300 dark:border-slate-700 cursor-pointer hover:opacity-90 transition-opacity rounded h-full"
                                        data-image-preview
                                        data-image-src="{{ $prestasiSiswa->gambar ? asset('storage/' . $prestasiSiswa->gambar) : '' }}">
                                        <img id="preview-img-gambar-prestasi"
                                            src="{{ $prestasiSiswa->gambar ? asset('storage/' . $prestasiSiswa->gambar) : '' }}"
                                            alt="Pratinjau" class="w-full h-full object-cover">
                                    </div>
                    ...
                    <div>
                        ...
                        <div id="drop-zone-sertifikat"
                            ...
                            <div id="upload-placeholder-sertifikat"
                                class="text-center space-y-3 w-full h-full flex flex-col items-center justify-center p-8 {{ $prestasiSiswa->foto_sertifikat ? 'hidden' : '' }}">
                                <svg class="w-16 h-16 text-gray-400 dark:text-slate-500 mx-auto" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div id="preview-container-sertifikat"
                                class="{{ $prestasiSiswa->foto_sertifikat ? '' : 'hidden' }} w-full h-full p-2">
                                <div class="relative group h-full">
                                    <div class="overflow-hidden border border-gray-300 dark:border-slate-700 cursor-pointer hover:opacity-90 transition-opacity rounded h-full"
                                        data-image-preview
                                        data-image-src="{{ $prestasiSiswa->foto_sertifikat ? asset('storage/' . $prestasiSiswa->foto_sertifikat) : '' }}">
                                        <img id="preview-img-sertifikat"
                                            src="{{ $prestasiSiswa->foto_sertifikat ? asset('storage/' . $prestasiSiswa->foto_sertifikat) : '' }}"
                                            alt="Pratinjau" class="w-full h-full object-cover">
                                    </div>
                    ...
                    <div>
                        <label for="judul" class="block text-base font-semibold text-slate-700 dark:text-white mb-1">Judul
                            <span class="text-red-600 dark:text-red-500">*</span></label>
                        <div class="relative">
                            <input type="text" name="judul" id="judul" value="{{ old('judul', $prestasiSiswa->judul) }}"
                                required maxlength="255" placeholder="Masukan Judul Prestasi"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" name="nama_siswa" id="nama_siswa"
                                value="{{ old('nama_siswa', $prestasiSiswa->nama_siswa) }}" required maxlength="255"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <textarea name="deskripsi" id="deskripsi" rows="6" maxlength="500"
                                ...
                                class="w-full border-2 border-gray-200 dark:border-slate-600 px-3 py-2.5 text-base bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:border-green-600 focus:outline-none rounded">{{ old('deskripsi', $prestasiSiswa->deskripsi) }}</textarea>
                        </div>
                ...
                <div class="space-y-6">
                    <div
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="text" name="kelas" id="kelas" value="{{ old('kelas', $prestasiSiswa->kelas) }}"
                                    required maxlength="50" placeholder="Masukan Kelas"
                        ...
                        <div>
                            ...
                            @php $jenis = old('jenis_prestasi', $prestasiSiswa->jenis_prestasi); @endphp
                            <select name="jenis_prestasi" id="jenis_prestasi" required
                                class="w-full bg-white dark:bg-slate-900 border-2 border-gray-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 px-3 py-1.5 text-base focus:border-green-600 focus:outline-none rounded">
                                <option value="Akademik" @selected($jenis === 'Akademik')>Akademik</option>
                                <option value="Non-Akademik" @selected($jenis === 'Non-Akademik')>Non-Akademik</option>
                            </select>
                        ...
                        <div>
                            <label for="tingkat_prestasi"
                                ...
                                Prestasi <span class="text-red-600">*</span></label>
                            @php $tingkat = old('tingkat_prestasi', $prestasiSiswa->tingkat_prestasi); @endphp
                            <select name="tingkat_prestasi" id="tingkat_prestasi" required
                                ...
                                @foreach(['Sekolah', 'Kecamatan', 'Kabupaten', 'Provinsi', 'Nasional', 'Internasional'] as $t)
                                    <option value="{{ $t }}" @selected($tingkat === $t)>{{ $t }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_prestasi" id="tanggal_prestasi"
                                value="{{ old('tanggal_prestasi', optional($prestasiSiswa->tanggal_prestasi)->format('Y-m-d')) }}"
                                required
                                class="w-full bg-white dark:bg-slate-900 border-2 border-gray-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 px-3 py-1.5 text-base focus:border-green-600 focus:outline-none rounded">
